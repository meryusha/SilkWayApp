...
public class TourProposalEditActivity extends AppCompatActivity {
    ...
    //lists
    private List<TourProposalPhoto> tourImages;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (tour == null || tourProposal == null) {
            finish();
        }
        initViews();
    }
    ...
    private void setToolbar() {
        ...
        toolbarTextView.setText("Встречное предложение");
    ...
    private void initPhotos() {
        ...
        tourImages = DataManager.getInstance().getCurrentTourProposalImages();
        if (tourImages != null) {
    ...
    private void initMainPhoto() {
        ...
        Glide.with(this).load(tourProposal.getAvatarUrl()).centerCrop().into(addTourImageView);
    ...
    private void onAddImageClickButton(final TourProposalPhoto tourPhoto) {
    ...
    private void deleteImage(TourProposalPhoto tourPhoto) {
        showDialog(getResources().getString(R.string.dialogTitleSavingInfo), getResources().getString(R.string.dialogMessage));
        Backendless.Persistence.of(TourProposalPhoto.class).remove(tourPhoto,
                new AsyncCallback<Long>() {
                    ...
                    public void handleFault(BackendlessFault fault) {
                        ...
                        DataManager.getInstance().showError(TourProposalEditActivity.this);
                    }
                });
    }
    ...
    private void saveTourProposalInBackend() {
        showDialog(getResources().getString(R.string.dialogTitleSavingInfo), getResources().getString(R.string.dialogMessage));
        ...
        for (int i = 0; i < imageViews.size(); i++) {
            ...
            if (imageBoolList.get(i)) {
                ...
                if (i < tourImages.size()) {
                    TourProposalPhoto photo = tourImages.get(i);
    ...
    private void uploadImage(Bitmap bitmap, final String description, final int index) {
        ...
        Backendless.Files.Android.upload(bitmap, Bitmap.CompressFormat.JPEG, 100, "item" + System.currentTimeMillis(), "items", new AsyncCallback<BackendlessFile>() {
                    ...
                    public void handleResponse(BackendlessFile response) {
                        ...
                        TourProposalPhoto photo;
                        if (index >= tourImages.size()) {
                            photo = new TourProposalPhoto();
                            photo.setTourProposal(newTourProposal);
                        } else {
                            photo = tourImages.get(index);
                        }
